...
const addNewDirector = (dname) => {
  return new Promise ((resolve, reject) => {
    connection.query(`insert into director_stats (d_name) values("${dname}")`);
    connection.query('select max(id) as id from director_stats', (error, results) => {
      if (error) {
        return reject(error);
      }
      return resolve(results);
...
const deleteDirector = (id) => {
  return new Promise ((resolve, reject) => {
    connection.query(`delete from director_stats where id=${id}`);
    connection.query(`select id from director_stats where id=${id}`, (error, results) => {
      if (error) {
        return reject(error);
      }
      return resolve(results);
